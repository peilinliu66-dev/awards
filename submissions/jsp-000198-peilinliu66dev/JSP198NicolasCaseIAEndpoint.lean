...
import JSP198NicolasCaseIANinePoint
...
theorem caseIA_first_outer_supports_last_edge
    ...
    orient (matchStart P hgp o ho run.last) (matchEnd P hgp o ho run.last)
  ...
  have acz : orient a c z < 0 := by
    ...
    convert hh using 1 <;> unfold orient <;> ring
  change orient c z (A : Point) < 0
  have nz : orient c z (A : Point) ≠ 0 := by
    have hh := gpSII2 A c z ecI ezI ec.2.2.1
    convert hh using 1 <;> unfold orient <;> ring
  by_contra hn
  have hbad : 0 < orient (A : Point) c z := by
    have hh : 0 < orient c z (A : Point) := lt_of_le_of_ne (le_of_not_gt hn) nz.symm
    convert hh using 1 <;> unfold orient <;> ring
  apply ia_nine_point_end_support_certificate A R K B a b c z o
    (by
    ...
    (by
      have he : orient (B : Point) K c = -orient (K : Point) B c := by unfold orient; ring
      rw [he]; linarith [right c ecI])
    rao ownR hfK.1 ownK aao noL noR hbad
    (gpSII A B c nAB ecI) (gpSII A K a nAK eaI)
    (gpSII A K c nAK ecI) (gpSII A K o nAK eoI) (gpSII A K z nAK ezI)
    (gpSII2 A a c eaI ecI nac) (gpSII2 A a z eaI ezI nza.symm)
    (gpSII K R a nKR eaI) (gpSII K R c nKR ecI) (gpSII K R z nKR ezI)
    (gpSII2 K a c eaI ecI nac) (gpSII2 K a o eaI eoI nao)
    (gpSII2 K a z eaI ezI nza.symm) (gpSII2 R a z eaI ezI nza.symm)

end JSP198.Nicolas
#print axioms JSP198.Nicolas.caseIA_first_outer_supports_last_edge
